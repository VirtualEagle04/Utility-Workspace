	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(descuento == null || descuento.isEmpty()) {
			...
			if (descuentoError == false) {
				...
				descuentoNum = desc;
				if (desc < 0 || desc > 25) {
					out.println("<li><p>El campo <b>'Descuento (%)'</b> contiene n&uacute;meros muy grandes. Rango: [0,25] </p></li>");
					error = true;
				}
			}
		}
		...
		if (error) {
			...
			out.println("<tr height='20px'></tr>");
			out.println("<tr><th>Precio Total:</th><td> $ " + ((precioNum * cantidadNum)-(descuentoNum * (precioNum * cantidadNum))/100) );
			out.println("</table>");
